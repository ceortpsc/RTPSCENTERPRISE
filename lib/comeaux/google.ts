...
export async function getGoogleServiceAccountToken(scopes: string[]) {
  ...
  const payload = b64url(JSON.stringify({ iss: clientEmail, scope: scopes.join(" "), aud, iat: now, exp: now + 3600 }));
  const unsigned = `${header}.${payload}`;
  const signer = createSign("RSA-SHA256"); signer.update(unsigned); signer.end();
  const assertion = `${unsigned}.${signer.sign(privateKey).toString("base64url")}`;
  const response = await fetch(aud,{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded"},body:new URLSearchParams({grant_type:"urn:ietf:params:oauth:grant-type:jwt-bearer",assertion}),cache:"no-store"});
...
export function merchantProductInput(product: { sku:string; name:string; description:string; priceCents:number; quantityOnHand:number; imageUrl:string; brand:string; productType:string; }) {
  const origin = process.env.NEXT_PUBLIC_COMEAUX_STORE_ORIGIN || "http://localhost:3000";
  const imageLink = product.imageUrl.startsWith("http") ? product.imageUrl : new URL(product.imageUrl, origin).toString();
  return {
    offerId: product.sku, contentLanguage: "en", feedLabel: "US",
    productAttributes: {
      title: product.name, description: product.description,
      link: `${origin}/comeaux-supply/products#${encodeURIComponent(product.sku)}`,
      imageLink, brand: product.brand, productTypes: [product.productType],
      condition: "NEW", availability: product.quantityOnHand > 0 ? "IN_STOCK" : "OUT_OF_STOCK",
      price: { amountMicros: String(product.priceCents * 10_000), currencyCode: "USD" }
    }
  };
}
