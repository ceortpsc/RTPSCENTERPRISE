...
import { PRODUCTS } from "@/lib/comeaux/store";
import { getGoogleServiceAccountToken, merchantProductInput } from "@/lib/comeaux/google";

const scope = "https://www.googleapis.com/auth/content";

function assertAdmin(req: NextRequest) {
  const provided = req.headers.get("x-comeaux-admin-key");
  const expected = process.env.COMEAUX_ADMIN_SYNC_KEY;
...
export async function POST(req: NextRequest) {
  ...
  if (!accountId || !dataSource) {
    return NextResponse.json({ error: "Merchant Center account/data source is not configured." }, { status: 503 });
  }
  ...
  return NextResponse.json({
    ...
    results
  }, { status: results.every(x => x.ok) ? 200 : 207 });
}
